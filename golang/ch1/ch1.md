...
这是Go特有的一种迭代结构，你会发现他在许多情况下都很有用。它可以迭代任何一个集合。语法上很类似其他语言中的`foreach`语句，但你依旧可以获得每次迭代所对应的索引。一般形式为下面这样：
...
需要注意的是，`val`始终为集合中对应索引的值拷贝，因此它一般只具有只读性质，对它所做的任何修改都不会影响到集合中原有的值，如果`val`为指针，则会产生指针的拷贝，依旧可以修改集合中的原值。

```go
func main() {
  m := map[string]string{"0": "A", "1": "B", "2": "C", "3": "D"}
...
`break`在`for`中使用时会退出当前的循环，在`switch`或`select`中则是跳过整个代码块，执行后面的代码。它可以用于任何的`for`循环（计数器、条件判断等等）。

下面的例子中包含了嵌套的循环体，`break`只会退出内层的循环：
...
//输出结果: 012345--012345--012345--
```

`continue`会忽略剩余的循环体而直接进入下一次循环的过程，但不是无条件执行下一次循环，执行之前依旧需要满足循环的判断条件。
...
另外，关键字`continue`只能用于`for`循环中。

### 标签和goto

`for`、`switch`或`select`语句都可以配合标签（label）形式的标识符使用，也就是某一行第一个以冒号（:）结尾的单词。
标签的名称是大小敏感的，为了提升可读性，一般建议使用全部大写字母。
...
这个例子中，`continue`语句指向`LABEL1`，当执行到该语句时，就会跳到`LABEL1`标签的位置。可以看到当`j==4`和`j==5`时，没有任何输出。标签的作用对应为外部循环，因此`i`会直接变成下一个循环的值，而此时`j`的值会被重置为`0`，也就是它的初始值。
如果把`continue`改成`break`，则不会只退出内循环，而是直接退出外循环。另外，还可以使用`goto`语句和标签配合使用来模拟循环。
...
使用逆向的`goto`会很快导致意大利面条式的代码，所以不要使用，应该选择更好的替代方案。

特别注意，不推荐使用标签和`goto`语句。它们会导致非常糟糕的程序设计，应选择更加可读的替代方案来实现相同的需求。
...
上面的代码中，由于在`goto`语句的下一行声明了一个变量，所以导致编译错误。